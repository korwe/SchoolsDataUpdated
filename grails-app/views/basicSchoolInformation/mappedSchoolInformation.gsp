...
<head>
    ...
    <script type="text/javascript">
        function initialize() {
            ...
            <g:if test="${ (i != 0) || (basicSchoolInformationInstanceTotal==1) }">
            //The first index is a dummy for housekeeping purposes. Disregard it.
            //Unless their is no other marker apart from the dummy.
            var schoolIndex=${i};
            ...
            if (!isNaN(lat) || !isNaN(lon)) {
                ...
                <g:if test="${ (i == 1) || ((i == 0) && (basicSchoolInformationInstanceTotal==1)) }">
                ...
                function getZoomedMarkersAjax(x) {
                    ...
                    ${remoteFunction( action:'testLatLong', params:"'edge=' + edge ", onSuccess:"onSuccess(data,textStatus)")};
                }
          ...
          <%--      google.maps.event.addListenerOnce(map, 'idle', function (e1) {
                   // alert('bc');
                        google.maps.event.addListener(map, 'bounds_changed', getZoomedMarkersAjax(this));
                });

                    google.maps.event.addListenerOnce(map, 'idle', function (e2) {
                       // alert('zc');
                        google.maps.event.addListener(map, 'zoom_changed', getZoomedMarkersAjax(this));
                });--%>
<%--try {
    google.maps.event.addListenerOnce(map, 'bounds_changed', function (e1) {
    google.maps.event.addListener(map, 'idle', getZoomedMarkersAjax(this));
...
<table>
    <tr>
        <td>
            ...
            <g:elseif test="${basicSchoolInformationInstanceTotal == 2}">
                <p>1 school displayed.</p>
            </g:elseif>
            <g:elseif test="${basicSchoolInformationInstanceTotal-1 < session['pageListSize']}">
                <p>${basicSchoolInformationInstanceTotal - 1} schools displayed.</p>
            </g:elseif>
            <g:elseif test="${basicSchoolInformationInstanceTotal-1 == session['pageListSize']}">
                <p>A random selection of ${basicSchoolInformationInstanceTotal - 1} schools displayed.</p>
            </g:elseif>
            <g:else>
                <script>
